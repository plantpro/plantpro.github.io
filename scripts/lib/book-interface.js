...
(function () {
  var CONTENT_LIST, appendDivision, appendDivisionHeader, buildContentList, createContentItem, hideContentList, hideSourceList, item, showOrHideContentList, showOrHideSourceList;

  hideContentList = function hideContentList() {
    return $("#content-list").hide();
  };

  hideSourceList = function hideSourceList() {
    $(".source-mark").hide();
    return $("#source-list").hide();
  };

  showOrHideSourceList = function showOrHideSourceList() {
    $("#source-list").fadeToggle();
    $(".source-mark").fadeToggle();
    return hideContentList();
  };

  showOrHideContentList = function showOrHideContentList() {
  ...
  $("#show-hide-content-list").click(showOrHideContentList); // Creates a content list item

  item = function item(name, href) {
    return {
      name: name,
      href: href
    };
  ...
  appendDivisionHeader = function appendDivisionHeader(name) {
    return $("#content-list").append($("<p class='px-3 pb-4 mb-0 mt-4'>".concat(name, "</p>")));
  };

  createContentItem = function createContentItem(_ref) {
    ...
    var a;
    a = $("<a href='../".concat(href, "'>").concat(index + 1, ". ").concat(name, "</a>"));
  ...
  appendDivision = function appendDivision(_ref3) {
    ...
    ref = _toConsumableArray(items.entries());
  ...
  buildContentList = function buildContentList() {
    var division, i, len, results;
    results = [];

    for (i = 0, len = CONTENT_LIST.length; i < len; i++) {
      division = CONTENT_LIST[i];
