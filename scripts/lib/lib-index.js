...
(function () {
  var autorOnClick, availableFileTypes, checkRecordForFilters, createRegExpFromSearchText, isSatisfiedToAllFilters, isSatisfiedToAutorFilter, isSatisfiedToFileTypeFilter, isSatisfiedToLanguageFilter, isSatisfiedToSearch, makeFilterPanel, makeFilterPanelWithColor, parseFileType, predicates, searchInputClear, showRecordIfSatisfiedToAllFilters, stateChanged, updateResults, updateSearchText;
  availableFileTypes = {
    PDF: {
      name: ".pdf",
  ...
  isSatisfiedToAutorFilter = function isSatisfiedToAutorFilter(record) {
    ...
    for (j = 0, len = ref.length; j < len; j++) {
      ...
      if (!record.getElementsByClassName("record-autor").some(function (elem) {
        return elem.innerText === autorName;
      })) {
        return false;
      }
    }
